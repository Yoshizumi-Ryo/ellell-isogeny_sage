...
def CodOne(tc_0:NullCoord,basis:list):
    ...
    h_lincom_lpow={key:Coord([h_lincom[key].numer[i]**l for i in range(0,4)],h_lincom[key].denom**l,tc_0.field) for key in h_lincom.keys()}
...
def EvalOne(tc_0:NullCoord,basis:list,pt_list:list,lmd_data:list):
    ...
    den_lpow=den**l
    xplincom_lpow={key:Coord([(xplincom[key].numer[i])**l for i in range(0,4)],den_lpow,tc_0.field) for key in xplincom.keys()}
