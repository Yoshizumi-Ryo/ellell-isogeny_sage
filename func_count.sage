...
def Count_prepare(p,count_from,count_max:int):
...
def Count_prepare_1mod4(p,count_from,count_max:int):
    for l in range(count_from,count_max+1):
...
def Count_Cod(count_max,N_A,pt_data,K,kind):
...
def Count_Eval(count_from,count_max,N_A,pt_data,K,kind):
    [tc_0,tc_f1,tc_f2,tc_f12,tc_x,tc_xpf1,tc_xpf2]=pt_data
    ell_list=[]
    e_list=[]
    for l in range(count_from,count_max+1):
...
def Count_Eval_1mod4(count_from,count_max,N_A,tc_0,tc_f1,tc_f2,tc_f12,tc_x,tc_xpf1,tc_xpf2,K,kind):
    ell_list=[]
    e_list=[]
    for l in range(count_from,count_max+1):
        if is_prime(l) and (l%4==1):
