...
from func_elliptic import Elliptic_Cyclic,Is_Elliptic_product,Theta_Hadamard,Leg_lv2,Product_theta,Legendre_to_lv2tnp,Elliptic_to_Legendre,Is_isomorphic_Legendre,Lv2tnp_to_Legendre,GFp4pow,Elliptic_to_Legendre_with_basis,Decomp_degree,Lv2_to_Legendre,Legendre_to_Elliptic
from class_theta import Coord,NullCoord
from func_isogeny import CodOne,Product_power_lambda,EvalOne
...
def Image_by_RepInt(E,rep_int:tuple,P,Q,end_i):
    """ 
    E_0:y^2=x^3+x / F_p.
...
def Auxiliary_path(E,N_A:int,N_B:int,P,Q):
    ...
    a=N_A-N_B
    inv_N_B=(ZZ(N_B)).inverse_mod(ZZ(N_A))
...
def Theta_Split(lv2tnp:list,lv2_x:list,lv2_y:list,zeta_4):
    ...
    if Is_Elliptic_product(lv2tnp)[1]:  #already split.
        ...
        i=Is_Elliptic_product(lv2tnp)[2] #(i,j) is zero even theta.
        j=Is_Elliptic_product(lv2tnp)[3]
    if (i,j)==(0,0):
        lv2tnp[2]*=zeta_4
        lv2tnp[3]*=zeta_4
        lv2_x[2] *=zeta_4
...
def Is_correct_isogeny(E_dm,E_cd,deg:int,ker,P_dm,Q_dm,P_cd,Q_cd):
    """ functions for splitting isogeny."""
...
def Construct_pt(ell_data:list,points:list,K):
    ...
    tc_x12=Coord([K(lv2_x12[i]) for i in range(0,4)],1,K)
    return tc_x12
...
def Attack_prepare(E_B,E_pr,N_A:int,N_B:int,PA_EB,QA_EB,alpha_PA,alpha_QA,K):
    """ 
    function for attack (1/3)
    We want to compute (N_A,N_A)-isogeny from  E_pr*E_B with kernel <f_1=(alpha_PA,PA_EB),f_2=(alpha_QA,QA_EB)>.
    In this function, we construct theta coordinates in dimension 2 for given points.
    """
    assert(PA_EB    in E_B)
    assert(QA_EB    in E_B)
    ...
    tc_0=NullCoord([K(lv2tnp[i]) for i in range(0,4)],1,K)
    ...
    ext_x=(tc_x,tc_xpf1,tc_xpf2)
    ext_y=(tc_y,tc_ypf1,tc_ypf2)
    return E_B_L,PA_EB,QA_EB,tc_0,ext_kernel_basis,ext_x,ext_y
...
def Attack_main(N_A:int,N_B:int,tc_0:NullCoord,ext_kernel_basis:tuple,ext_x:tuple,ext_y:tuple):
    """ 
...
def Attack_total(E_0m,E_B,E_pr,N_A:int,N_B:int,P_A,Q_A,PA_EB,QA_EB,alpha_PA,alpha_QA,zeta_4,K):
    """ function combined (1/3)->(2/3)->(3/3)."""
    assert(order(P_A)==order(Q_A)==order(PA_EB)==order(QA_EB)==order(alpha_PA)==order(alpha_QA)==N_A)
    assert(gcd(N_A,N_B)==1)
    #(1/3)
    E_B_L,PA_EB,QA_EB,tc_0,ext_kernel_basis,ext_x,ext_y=Attack_prepare(E_B,E_pr,N_A,N_B,PA_EB,QA_EB,alpha_PA,alpha_QA,K)
    #(2/3) ## Main!! ##
    lv2tnp,lv2_ZS1,lv2_ZS2=Attack_main(N_A,N_B,tc_0,ext_kernel_basis,ext_x,ext_y)
    #(3/3)
