...
def Attack_prepare(E_B,E_pr,N_A,N_B,PA_EB,QA_EB,alpha_PA,alpha_QA,K):
    assert(PA_EB    in E_B)
    assert(QA_EB    in E_B)
    ...
    lm_pr,E_pr_L,iso_Epr_EprL=Elliptic_to_Legendre(E_pr,K)
    lm_B,E_B_L,iso_EB_EBL,S1,S2=Elliptic_to_Legendre_with_basis_2(E_B,N_B,K)
    #print("take bais fin")
    ...
    print("critical pt",sq_rt_lm_pr==sq_rt_lm_B)
    lv2tnp=Product_theta(lv2tnp_Epr,lv2tnp_EB)
    tc_0=NullCoord(lv2tnp,1)
    ...
    tc_f1   =Construct_pt(ell_data,f_1)
    tc_f2   =Construct_pt(ell_data,f_2)
    tc_f12  =Construct_pt(ell_data,[f_1[0]+f_2[0],f_1[1]+f_2[1]])
    tc_x    =Construct_pt(ell_data,x)
    tc_xpf1 =Construct_pt(ell_data,[x[0]  +f_1[0],x[1]  +f_1[1]])
    tc_xpf2 =Construct_pt(ell_data,[x[0]  +f_2[0],x[1]  +f_2[1]])
    tc_y    =Construct_pt(ell_data,y)
    tc_ypf1 =Construct_pt(ell_data,[y[0]  +f_1[0],y[1]  +f_1[1]])
    tc_ypf2 =Construct_pt(ell_data,[y[0]  +f_2[0],y[1]  +f_2[1]])
...
def Attack_main(N_A,N_B,tc_0,tc_f1,tc_f2,tc_f12,tc_x,tc_xpf1,tc_xpf2,tc_y,tc_ypf1,tc_ypf2):
    assert(gcd(N_A,N_B)==1)
    assert(tc_0.Is_order(tc_f1  ,N_A))
    assert(tc_0.Is_order(tc_f2  ,N_A))
    assert(tc_0.Is_order(tc_f12 ,N_A))
    ...
    for i in range(0,len(fac)):
        ...
        assert(s*l*k==N_A)
        print("l=",l)
        print(Is_Ellipitc_product(tc_0.numer)[0])
        ...
        #-----------------------------------------------------------------------------------------
        lmd_pow_product=Product_power_lambda(tc_0,basis)
        tc_cd0_c1=Codomain_C1(tc_0,basis)
        tc_cd0_c2=Codomain_C2(tc_0,basis)
        tc_cd0_c3=Codomain_C3(tc_0,basis)
        
        print("c1_c2",tc_cd0_c1.Is_same_proj(tc_cd0_c2))
        print("c2_c3",tc_cd0_c2.Is_same_proj(tc_cd0_c3))
        print("c3_c1",tc_cd0_c3.Is_same_proj(tc_cd0_c1))
        
        tc_f1 =Evaluation_E2(tc_0,basis,f1_list,lmd_pow_product)
        tc_f2 =Evaluation_E2(tc_0,basis,f2_list,lmd_pow_product)
        tc_x_e1=Evaluation_E1(tc_0,basis,x_list ,lmd_pow_product)
        tc_x=Evaluation_E2(tc_0,basis,x_list ,lmd_pow_product)
        tc_x_e3=Evaluation_E3(tc_0,basis,x_list ,lmd_pow_product)
        tc_y  =Evaluation_E2(tc_0,basis,y_list ,lmd_pow_product)
        tc_0 =tc_cd0_c1
        s*=l
        print("e1_e2",tc_x_e1.Is_same_proj(tc_x   ))
        print("e2_e3",tc_x   .Is_same_proj(tc_x_e3))
        print("e3_e1",tc_x_e3.Is_same_proj(tc_x_e1))
        
        print("be false",Is_Ellipitc_product(tc_0.numer)[0])
...
def Attack_last(E_0m,N_A,N_B,P_A,Q_A,PA_EB,QA_EB,E_B_L,lv2tnp,lv2_ZS1,lv2_ZS2,zeta_4,K):
    ...
    lm_0=Elliptic_to_Legendre(E_0m,K)[0]
...
def Attack_total(E_0m,E_B,E_pr,N_A,N_B,P_A,Q_A,PA_EB,QA_EB,alpha_PA,alpha_QA,zeta_4,K):
    assert(order(P_A)==N_A)
    assert(order(Q_A)==N_A)
    assert(order(PA_EB)==N_A)
    assert(order(QA_EB)==N_A)
