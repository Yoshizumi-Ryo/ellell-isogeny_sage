...
def Sum_of_square_for_E2(l:int):
    assert(is_prime(l))
    assert(l!=2)
    if l==3:
        return (1,1,1)
...
def Evaluation_E1(tc_0:Coord,basis:list,pt_list:list,lmd_data:list):
    l=basis[0].order
...
def Evaluation_E2(tc_0:NullCoord,basis:list,pt_list:list,lmd_data:list):
    l=basis[0].order
    [lmd_pow_product,lm_1_lsq,lm_2_lsq]=lmd_data
    assert(type(lmd_pow_product)==dict)
    assert(type(lm_1_lsq)==list)
