...
def In_subfield(a):
    """ check if the given element is in F_{p^2}."""
    K=parent(a)
    p=K.characteristic() 
    assert(order(K)==p**4)
    assert(a in K)
...
def In_subfield_tc(tc:Coord):
    """ check if the given theta coordinate is in F_{p^2}."""
...
#theta function on elliptic curve========================================================
...
def Elliptic_to_Legendre_with_basis(E,N:int):
    """ 
    For given E, compute lmd s.t. y^2=x*(x-1)*(x-lmd).
    In addition, we get N-torsion basis of E.
    """ 
    coff=E.a_invariants()
    #y^2=x^3+ax^2+bx+c
    K=E.base_ring()
...
def Leg_lv2(lm,sq_rt_lm,sq_rt_lmm1,lv2tnp:list,pt):
    """ 
    From Legendre representation, transform to lv2 theta coordinate.
    """
    assert(sq_rt_lm**2==lm)
    assert(sq_rt_lmm1**2==lm-1)
    u=pt[0]
...
def Lv2_to_Legendre(lm,sq_rt_lm,sq_rt_lmm1,lv2tc:list):
    """ 
    From lv2 theta coordinate, compute Legendre representation.
    """
    r=lv2tc[1]/lv2tc[0]
...
def Is_Elliptic_product(lv2tnp):
    """ 
    ...
    if lv22tnpsq[(3,3)]==0:
        return True,True #elliptic product with product theta structure.
